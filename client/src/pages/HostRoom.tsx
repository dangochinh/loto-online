    ...
    return (
        <div className="flex flex-col h-screen overflow-hidden bg-slate-900 text-white">
            ...
            <header className="bg-slate-800 shadow-md z-10 px-2 md:px-4 py-2 md:py-3 flex flex-wrap md:flex-nowrap items-center justify-between gap-1.5 md:gap-3">
                ...
                {/* Middle group: Voice + Speed (shown on right side of row 1 on mobile) */}
                <div className="flex items-center gap-1.5 order-last md:order-none w-full md:w-auto justify-between md:justify-start">
                    {/* Mobile-only action buttons */}
                    <div className="flex gap-1.5 md:hidden">
                        <button
                            onClick={() => setShowPlayerListMobile(!showPlayerListMobile)}
                            className={clsx("px-2.5 py-1.5 rounded-lg font-bold text-sm", showPlayerListMobile ? "bg-cyan-600 text-white" : "bg-slate-700 text-slate-300")}
                            title="Toggle Players"
                        >👥</button>
                        <button onClick={() => setShowAllTickets(true)} className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 rounded-lg font-bold text-sm" title="Xem Vé">👁️</button>
                        <button onClick={() => setShowHistory(true)} className="px-2.5 py-1.5 bg-slate-700 hover:bg-slate-600 border border-slate-600 rounded-lg font-bold text-sm" title="History">📜</button>
                    </div>

                    {/* Mobile-only game action buttons */}
                    <div className="flex gap-1.5 md:hidden">
                        {gameState === 'WAITING' && (
                            <button onClick={() => actions.startGame()} disabled={players.length === 0 || !players.every(p => p.isReady)}
                                className={clsx("px-3 py-1.5 rounded-lg font-bold text-sm transition-all",
                                    players.length > 0 && players.every(p => p.isReady) ? "bg-green-600 hover:bg-green-700 shadow-lg" : "bg-slate-700 text-slate-500 cursor-not-allowed opacity-50"
                                )}>
                                {players.length === 0 ? "Đợi..." : "▶️"}
                            </button>
                        )}
                        {gameState === 'PLAYING' && (
                            <button onClick={actions.pauseGame} className="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 rounded-lg font-bold text-sm">⏸️</button>
                        )}
                        {gameState === 'PAUSED' && (
                            <button onClick={actions.resumeGame} className="px-3 py-1.5 bg-green-600 hover:bg-green-700 rounded-lg font-bold text-sm">▶️</button>
                        )}
                        <button onClick={() => setShowExitConfirmation(true)} className="px-2.5 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm" title="Exit">🚪</button>
                        <button onClick={() => setShowIntro(true)} className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white shadow-lg border border-orange-300/50" title="Info">
                            <span className="text-lg font-bold font-serif italic">i</span>
                        </button>
                    </div>
                </div>

                {/* Right group: Controls (visible on all sizes) */}
                <div className="flex items-center gap-1.5">
                    {/* Mute */}
                    <button
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 p-2 md:p-4 flex flex-col">
                    ...
                    <div className="grid grid-cols-9 gap-1 md:gap-1.5 max-w-2xl w-full mx-auto flex-1">
                        {Array.from({ length: 90 }, (_, i) => {
                            ...
                            return (
                                <div
                                    key={num}
                                    className={clsx(
                                        "flex items-center justify-center rounded font-medium text-base md:text-lg transition-all duration-500",
